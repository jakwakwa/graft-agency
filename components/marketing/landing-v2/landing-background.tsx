import svgPaths from "@/imports/Landing/svg-3bp3v10rhn";

export function LandingBackground() {
  return (
    <div className="pointer-events-none blur-2xl -top-120 absolute inset-0 z-0 overflow-hidden" aria-hidden>
      <svg
        role="presentation"
        className="absolute top-0 left-0 h-full w-full opacity-40 mix-blend-screen"
        fill="none"
        preserveAspectRatio="none"
        viewBox="0 0 979 583"
      >
        <path d={svgPaths.p53f4d80} fill="#053DB5" fillOpacity="0.45" />
      </svg>
      <svg
        role="presentation"
        className="absolute top-[20%] right-[-50%] h-[2123px] w-[1065px] opacity-40 mix-blend-screen"
        fill="none"
        preserveAspectRatio="none"
        viewBox="0 0 1065 623"
      >
        <path d={svgPaths.p9436980} fill="#F90A22EC" fillOpacity="0.1" />
        <path d={svgPaths.p9436980} fill="#8C045AD0" fillOpacity="0.45" />
      </svg>
    </div>
  );
}
