...
export const createReserveSlotTool = (clientId: string) =>
  tool({
    description:
      "Temporarily reserves a slot in the calendar to prevent others from booking it while the visitor is finishing the booking process. Defaults to 5 minutes.",
    inputSchema: z.object({
      ...
      slotDuration: z
        .number()
        .optional()
        .describe("Slot duration in minutes (optional, defaults to event type length)"),
      reservationDuration: z
        .number()
        .optional()
        .describe("How many minutes the slot should be reserved for (defaults to 5 minutes)"),
    }),
    execute: async (input) => {
