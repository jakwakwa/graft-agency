...
overview: ""
todos:
  - id: todo-1776800517156-hbl32laud
    content: set ENGAGEMENT_DRY_RUN=true in local env. Confirm Gemini + Stitch API keys are live.
    status: completed
  - id: todo-1776799770367-06xkzedah
    content: Change app/api/engagement/status/[leadId]/route.ts Return full artifact payloads
    status: completed
  - id: todo-1776800083508-xiyvdwmy8
    content: app/(marketing)/dashboard/automation/queue/[id]/_components/engagement-panel.tsx Collapsible sections for needs, PRD, designs. Make PRD + design concepts + profiled needs readable in the dashboard. No pipeline logic changes.
    status: completed
  - id: todo-1776800318038-fhurge4ah
    content: app/(marketing)/dashboard/automation/queue/[id]/_components/engagement-panel.tsx — add collapsible sections under the stepper
    status: completed
  - id: todo-1776800354495-nfagfimws
    content: "collapsible section 1 - Profiled needs: render JSON as readable key/value cards (pain points, ICP fit, signals)."
    status: completed
  - id: todo-1776800363150-hwci1obbm
    content: "collapsible section 2 - PRD preview: render prdContent with a markdown renderer (check if react-markdown or an existing prose component is already in the repo before adding a dep)."
    status: completed
  - id: todo-1776800341305-py3k4tm6g
    content: "collapsible section 3 - Design concepts: 3-up grid showing concept name + link per concept"
    status: completed
  - id: todo-1776800094894-b3amxlg3d
    content: "lib/inngest/functions/jules-builder.ts - Gate any Vercel-project-link step behind ENGAGEMENT_DRY_RUN; Jules + GitHub calls run as normal "
    status: completed
  - id: todo-1776800438399-8f7qfopmf
    content: lib/inngest/functions/offer-dispatcher.ts Defensive dry-run early return - scripts/dry-run-engagement.ts (new) - Synthetic lead harness ENGAGEMENT_DRY_RUN
    status: completed
isProject: false
---
...
### Stub deploy/billing/email + dry-run harness 
...
1. Confirm Gemini + Stitch API keys are live.
