...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ clientId: string }> }) {
  ...
  const origin = process.env.NEXT_PUBLIC_APP_URL ?? "https://graft.today";

  const loaderScript = `
(function(){
  if(document.getElementById('graft-today-widget-container')) return;
  var c=document.createElement('div');
  c.id='graft-today-widget-container';
  c.style.cssText='position:fixed;bottom:24px;right:24px;z-index:2147483647;';
  var btn=document.createElement('button');
  btn.id='graft-today-widget-toggle';
  ...
  frame.id='graft-today-widget-iframe';
